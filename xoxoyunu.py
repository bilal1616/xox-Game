...
class Application(tk.Frame):
	...
	def draw_header_frame(self):
		...
		self.player_label = tk.Label(self.player_frame, text=f'Mevcut Oyuncu : {current}', fg='green',
	...
	def draw_text(self, event):
		...
		if self.check_space(cell):
			...
			if self.check_winner('X'):
				...
				self.after(800, lambda : self.game_over_window('   X Galip'))
			elif self.check_winner('O'):
				self.o_score += 1
				self.o_label['text'] = f'O   {self.o_score}'
				self.after(800, lambda : self.game_over_window('   O Galip'))

			if self.isBoardFull():
				self.after(800, lambda : self.game_over_window(' Oyun Berabere'))
			else:
				self.current_player = (self.current_player + 1) % 2
				self.player_label['text'] = f'Mevcut Oyuncu : {self.players[self.current_player]}'
	...
	def game_over_window(self, msg):
		...
		ttk.Button(self.top, text='Tekrar Oyna', command=self.restart_game, 
				width=10).grid(row=1, column=0, columnspan=2, pady=20, padx=20)

		ttk.Button(self.top, text='Çıkış', command=self.master.destroy, 
